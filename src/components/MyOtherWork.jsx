...
const MyOtherWork = () => {
  ...
  const handleNext = () => {
    ...
    animateSlide("left", () => {
      setStartIndex((prev) =>
        Math.min(prev + 1, myOtherWorkData.length - itemsToShow)
      );
    });
  };
  ...
  const visibleWorks = myOtherWorkData.slice(
    startIndex,
  ...
  return (
    <div className="bg-black text-white py-8 px-4">
      ...
      <div
        ref={containerRef}
        className="flex justify-center items-center space-x-3 md:space-x-32 mt-6"
      >
    ...
    ${index % 2 === 0 ? "rounded-t-full" : "rounded-b-full"}`}
          >
            <div
              className=" overflow-hidden w-24 h-36 md:h-[250px] md:w-64  mx-auto mb-2 hover:scale-105 transition-transform border hover:border-amber-300
    rounded-tr-[50px] rounded-bl-[50px]"
            >
              <img
                src={work.image}
                alt={work.title}
                className="w-full h-full object-scale-down hover:object-cover opacity-50 grayscale
             hover:opacity-100 hover:grayscale-0 transition duration-300"
              />
            </div>
            <h3 className="text-xs md:text-sm md:font-medium text-center">
              {work.title}
            </h3>
            <p className="hidden md:block text-xs mt-1 px-2">
              {work.description}
            </p>
            ...
            <div className="flex justify-center space-x-4 mt-2 text-blue-400 text-xs">
              <a
                ...
                className="hover:underline font-thin md:text-xl"
              >
                GitHub
              </a>
              {work.live && (
